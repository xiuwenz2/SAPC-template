...
from .uppercase_abbreviations import uppercase_abbreviations
...
class EnglishNumberNormalizer:
    ...
    def postprocess(self, s: str):
        ...
        # but NOT when 1 is part of a decimal number (e.g., "1.30")
        # Use negative lookahead and lookbehind to avoid decimals
        s = re.sub(r"(?<!\d)(?<!\.)1(s?)(?!\.)(?!\d)\b", r"one\1", s)
...
class EnglishTextNormalizer:
    ...
    def __init__(self, english_spelling_mapping=english_spelling_normalizer):
        ...
        self.replacers = {
            ...
            r"\bmr\b": "mister ",
            r"\bmrs\b": "missus ",
            # r"\bst\b": "saint ",  # handled separately by _disambiguate_st()
            r"\bdr\b": "doctor ",
            r"\bprof\b": "professor ",
            r"\bcapt\b": "captain ",
            r"\bgov\b": "governor ",
            ...
            r"n't\b": " not",
            r"'re\b": " are",
            # r"'s\b": " is",  # <--- REMOVED THIS LINE
            r"'d\b": " would",
            r"'ll\b": " will",
            r"'t\b": " not",
            r"'ve\b": " have",
            r"'m\b": " am",
            r"'em\b": " them",  # <--- ADDED THIS LINE BACK IN
            r"\.{2,}": " ",  # normalize multiple dots/ellipses to space
        }
        self.standardize_numbers = EnglishNumberNormalizer()
        self.standardize_spellings = EnglishSpellingNormalizer(english_spelling_mapping)

    def _merge_uppercase_abbreviations(self, text: str) -> str:
        """
        Merge adjacent single-character words into one word.
...
_S_CONTRACTION_PATTERN = re.compile(r"\b(\w+)'s\b", flags=re.IGNORECASE)
