...
def collect_records(json_dir: Path):
    ...
    for speaker_dir in tqdm(sorted(speaker_dirs), desc="Processing speaker directories"):
        ...
        for jp in sorted(json_files):
            ...
            for rec in extract_records_from_json(jp):
                ...
                if wav not in merged:
                    ...
                    prev = merged[wav]
                    if prev["raw_trans"] != rec["raw_trans"] or prev.get("etiology", "") != rec.get("etiology", ""):
                        print(f"[WARN] cross-file conflict for {wav}; keeping first seen, ignoring {jp}")
    
    print(f"[INFO] Processed {json_count} JSON files from {len(speaker_dirs)} speaker directories")
...
def build_manifest_from_txt(
    ...
    print(f"[INFO] Getting durations for {len(valid_entries)} files using {workers} workers...")
    duration_map = {}
    with ThreadPoolExecutor(max_workers=workers) as executor:
        future_to_path = {executor.submit(get_duration_safe, entry["path"]): entry for entry in valid_entries}
        for future in tqdm(as_completed(future_to_path), total=len(valid_entries), desc="Getting durations"):
    ...
    if missing_files or missing_transcript_files:
        with open(log_file, 'w', encoding='utf-8') as f:
            ...
            f.write(f"Total missing: {len(missing_files) + len(missing_transcript_files)}\n\n")
        ...
        print(f"[INFO] Wrote {total_missing} missing files ({len(missing_transcript_files)} missing transcripts, {len(missing_files)} missing audio) to {log_file}")
...
def parse_args():
    ap = argparse.ArgumentParser(description="Generate manifest CSV from JSON files and txt file.")
    ap.add_argument("--txt", required=True, help="Input txt file path")
    ap.add_argument("--json-dir", required=True, help="Directory containing JSON files (processed/<split>)")
    ...
    ap.add_argument("--workers", type=int, default=32, help="Number of parallel workers for getting durations")
