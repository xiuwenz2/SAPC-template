...
The ingestion program processes each audio file with **two threads**:

1. **Audio Sender thread**: reads the wav file and sends 100ms chunks to a queue.
2. **Decoder thread**: receives chunks and calls your model's `accept_chunk()` / `input_finished()`.
...
2. Implement the 4-method interface in `model.py`.
3. Package into a zip:

```
submission.zip
├── model.py              # Required (4-method streaming interface)
...
- Time limit: 21600 seconds (Final) per submission
